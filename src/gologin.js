...
import requestss from 'requestretry';
...
import cached_request from "cached-request";

const cachedRequest = cached_request(requestss)
...
const CACHE_DIR = process.env.DOCKER_VOLUME_PATH || './cache';
...
export class GoLogin {
    ...
    async getTimeZone(proxy) {
        ...
        if (proxy !== null && proxy.mode !== 'none') {
            ...
            data = await requests.get('https://ipgeo.gologin.com', {
                proxy: proxyUrl,
                timeout: 20 * 1000,
                maxAttempts: 5
            });
